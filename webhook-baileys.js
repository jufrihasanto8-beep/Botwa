...
Minta customer konfirmasi apakah sudah transfer ke rekening yang benar: ${userRekening || '(belum diisi)'}.`;
    ...
    if (orderConfirmed) {
      ...
      // ── Kirim konfirmasi pesanan ke customer ─────────────────
      try {
        const convFull2    = await sbGet('conversations', `?id=eq.${conversation.id}&limit=1`);
        const latestState2 = convFull2[0]?.state || {};
        const ongkirData2  = latestState2.ongkir || convState.ongkir;
        const area2        = ongkirData2?.area || {};
        const isCOD2       = (orderDataParsed.metode || latestState2.metode_bayar || 'COD').toLowerCase() !== 'transfer';
        const qty2         = orderDataParsed.qty  || latestState2.qty  || 1;
        const harga2       = ongkirData2?.harga      || 0;
        const ongkirPromo2 = ongkirData2?.ongkirPromo || 0;
        const feeCOD2      = ongkirData2?.feeCOD     || 0;
        const ekspLabel2   = (ongkirData2?.ekspedisi  || 'KURIR').toUpperCase();
        const total2       = isCOD2 ? (harga2 + ongkirPromo2 + feeCOD2) : (harga2 + ongkirPromo2);

        const customerConfirmMsg =
`✅ *Konfirmasi Order ${product?.nama || 'Produk'}*

*Nama:* ${customer?.nama || '-'}
*No HP:* ${customer?.wa_number || '-'}
*Alamat Lengkap:* ${orderDataParsed.alamat || latestState2.alamat || '-'}

*Kelurahan/Desa:* ${area2.kelurahan || '-'}
*Kecamatan:* ${area2.kecamatan || '-'}
*Kabupaten:* ${area2.kota || '-'}
*Provinsi:* ${area2.provinsi || '-'}
*Kode Pos:* ${area2.kodePos || '-'}

*Produk:* ${product?.nama || '-'}
*Jumlah Pesanan:* ${qty2} pcs
*Pembayaran:* ${isCOD2 ? `COD via ${ekspLabel2}` : `Transfer via ${ekspLabel2}`}
*Total Pembayaran:* Rp ${total2.toLocaleString('id-ID')}

Sudah bener kak? Agar bisa kami proses pengiriman 🙏`;

        await sendWA(userId, reply_jid, customerConfirmMsg);
        console.log(`Konfirmasi pesanan terkirim ke customer ${wa_number}`);
      } catch(e) {
